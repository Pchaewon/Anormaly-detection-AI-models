...
from model.core import helpers
...
def heatmap(real: torch.Tensor, recon: torch.Tensor, mask, filename, save=True):
    ...
    if save:
        ...
        plt.imshow(helpers.gridify_output(output, 5)[..., 0], cmap="gray")
        plt.axis('off')
...
def testing(testing_dataset_loader, diffusion, args, ema, model):
    ...
    for i in [*range(100, args['sample_distance'], 100)]:
        ...
        imgs = [[ax.imshow(helpers.gridify_output(x, row_size), animated=True)] for x in out]
